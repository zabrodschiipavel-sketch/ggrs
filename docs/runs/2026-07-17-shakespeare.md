...
- Время: **1244 с** (≈20.7 мин) — из вывода `cargo test` (`finished in 1244.12s`), это
  надёжная цифра. Реальный throughput = 512000 токенов / 1244.12с ≈ **412 ток/с**.
  ⚠️ **Поправка задним числом**: изначально здесь стояло «813 341 ток/с» из лога тренера —
  это было НЕВЕРНО. При калибровке T14 (см. `docs/runs/2026-07-17-tinystories-calibration.md`)
  обнаружен баг trainer.rs: `tok_per_s` делил кумулятивные `tokens_seen` на elapsed ОДНОГО
  шага (таймер сбрасывался каждую итерацию) вместо кумулятивного времени — печатал числа,
  завышенные примерно в ×1000-2000 и растущие с номером шага. Исправлено в `8bd9d98`.
  На loss/веса/резюм это не влияло (чисто информационное поле), но было честной ошибкой
  в этом отчёте — задним числом поправлено на пересчёт по надёжному времени `cargo test`.
