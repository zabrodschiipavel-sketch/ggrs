...
| Ставка | Что это | Ниша занята? | Шанс на резонанс |
|--------|---------|--------------|------------------|
| **CPU-спидран** | Лидерборд «TinyStories за N минут на ноутбучном CPU», ggrs — референс-движок | Нет (nanoGPT-спидран — только GPU) | Высокий |
| **Квант-обучение без f32** | Веса никогда не разжимаются: int-блоки + обучаемые scale/offset (F4-математика) | Почти нет (QLoRA морозит базу; QAT держит f32-мастера) | Средний, риск расходимости |
| **Self-evolution демон** | Ночной Proposer→Verifier→ZPD→LoRA цикл одной командой | В статьях есть, инструментом — нет | Средний |

Спидран — главная: он дешёвый на нашем железе, воспроизводимый, и становится площадкой для скрининга остальных идей (Muon, квант-обучение, куррикулум) за часы, а не недели.
